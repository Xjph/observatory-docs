...
- `system` (collection): See [Collections and Iterators](#collections-and-iterators).
- `parentsTable` (collection): A table of parent bodies indexed by BodyId.
...
This was expanded on in Explorer 1.3 with a `notifyForBody` which works similarly to `notify` but allows the user to specify which body the notification is in regard to. The function signature is `notifyForBody(string title, string detail, string extendedDetail, int bodyId)`.
